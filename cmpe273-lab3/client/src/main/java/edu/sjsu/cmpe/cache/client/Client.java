...
import com.google.common.hash.Hashing;
 ...
 int hsh = hashFunction.hashLong(key).asInt();
 ring.remove(hsh);
 }
 ...
 int hsh = hashFunction.hashLong((Integer) key).asInt();
 if (!ring.containsKey(hsh)) {
 SortedMap<Integer, String> tailMap = ring.tailMap(hsh);
 hsh = tailMap.isEmpty() ? ring.firstKey() : tailMap.firstKey();
 }
 return ring.get(hsh);
 }
 }
